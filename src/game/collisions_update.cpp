...
void collisions_Sensors_PlatformPaths_update_Position_System(flecs::iter &it, Position *positions, Sensors *sensorCollections, Velocity *velocities, GroundSpeed *groundSpeeds, GroundMode *groundModes, StateCurrPrev *states, Angle *angles ){
    ...
    for(u64 i : it){
        ...
        if(state == STATE_ON_GROUND){
            sensorCollections[i].rays[SENSOR_LEFT_FLOOR].distance = 32;
            sensorCollections[i].rays[SENSOR_RIGHT_FLOOR].distance = 32;
            if(groundModes[i] == GROUND_MODE_FLOOR){
                positions[i].y = 
                    highestIntersectingPoint.y - sensorCollections[i].rays[SENSOR_LEFT_FLOOR].distance/2;
                ...
                positions[i].x = 
                    highestIntersectingPoint.x + sensorCollections[i].rays[SENSOR_LEFT_FLOOR].distance/2;
